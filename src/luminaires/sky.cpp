...
public:
	...
	SkyLuminaire(const Properties &props)
        ...
        m_luminaireToWorld = Transform::rotate(Vector(1, 0, 0),-90) * props.getTransform("toWorld", Transform());
        ...
        Float timeOfDay = props.getFloat("timeOfDay", 22.00);
    ...
    void configureSunPosition(const Float lat, const Float lon,
        ...
        const Float solarTime = timeOfDay
                                    + (0.170 * sin(4 * M_PI * (julDay - 80)/373)
                                    - 0.129 * sin(2 * M_PI * (julDay - 8)/355))
                                    + (stdMrd - lon) / 15.0;
        ...
        const Float solarAltitude = asin(sin(degToRad(lat))
                ...
                * cos(solarDeclination) * cos(M_PI * solarTime / 12));

        const Float opp = -cos(solarDeclination) * sin(M_PI * solarTime / 12);
        const Float adj = -(cos(degToRad(lat)) * sin(solarDeclination)
                + sin(degToRad(lat)) * cos(solarDeclination)
                * cos(M_PI * solarTime / 12));
...
private:
    ...
    */
    ...
    void getSkySpectralRadiance(const Float theta, const Float phi, Spectrum &dstSpect) const {
        ...
        const Float Y = perezFunction(m_perezL, theta_fin, gamma, m_zenithL);
        // Convert xyY to XYZ
        ...
        // create spectrum from RGB values with exponential scaling
        dstSpect.fromLinearRGB(
            1.0 - exp(-(1.0/15000.0) * ( 3.240479 * X - 1.537150 * Y - 0.498535 * Z)),
            1.0 - exp(-(1.0/15000.0) * (-0.969256 * X + 1.875992 * Y + 0.041556 * Z)),
            1.0 - exp(-(1.0/15000.0) * ( 0.055648 * X - 0.204043 * Y + 1.057311 * Z)));
    }
